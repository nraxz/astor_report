...
class order_details_xls
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          { 
              ...
              $this->Res_xls = new order_details_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_order-details.zip";
          $this->Arquivo   .= "_order-details" . $this->Xls_tp;
          $this->Tit_doc    = "order-details" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_return']);
          if ($this->Tem_xls_res) {
      ...
      {
          $this->sum_persons = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][2];
          $this->sum_tot = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][3];
      }
   }
   //---- 
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['SC_Ind_Groupby'] == "bookingday")
         {
          ...
          {  
              ...
              if ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida'] && !$prim_gb && $this->groupby_show == "S") {
...
$_SESSION['scriptcase']['order-details']['contr_erro'] = 'off'; 
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida'])
         { 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['nolabel'])
       {
 ...
 function quebra_bookingday_bookingday($bookingday) 
 {
   ...
   function quebra_bookingday_bookingday_bot()
   {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['field_order'] as $Cada_cmp)
       {
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['embutida']) {
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = "";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
   ...
   function quebra_geral_bookingday_bot()
   {
       ...
       $prim_cmp = true;
       $mens_tot = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][0] . "(" . $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][1] . ")";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['field_order'] as $Cada_cmp)
       {
           if ($Cada_cmp == "persons" && (!isset($this->NM_cmp_hidden['persons']) || $this->NM_cmp_hidden['persons'] != "off"))
           {
               $Format_Num = "#,##0";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][2];
               $prim_cmp = false;
           ...
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['tot_geral'][3];
               $prim_cmp = false;
               if (!NM_is_utf8($Vl_Tot)) {
                   $Vl_Tot = sc_convert_encoding($Vl_Tot, "UTF-8", $_SESSION['scriptcase']['charset']);
               }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_file'] = $this->Xls_f;
      }
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['order-details']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
